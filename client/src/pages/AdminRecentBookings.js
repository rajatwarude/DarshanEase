// clien/src/pages/AdminRecentBookings.js
...
const AdminRecentBookings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get("/bookings/admin/recent", {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(135deg, #e3f2fd 0, #ffffff 40%, #e8f5e9 100%)",
        borderRadius: "16px",
        padding: "24px 28px",
        minHeight: "70vh",
      ...
      {bookings.map((b) => (
        <div key={b._id} className="card mb-3 shadow-sm">
          <div className="card-body">
            ...
            <p className="mb-1" style={{ fontSize: "13px" }}>
              <strong>Date:</strong> {new Date(b.slot?.date).toLocaleDateString()}{" "}
